...
sigma = float(sigma) * float(alpha) * 1000  # get dispersal distance in (m)
...
for i, frame in enumerate(files[:-1]):
    ...
    title.set_y(1.02)
    sim_param = r'$\rho = {0}, \ \ \beta = {1}\ (day^{2}),\ \ \ell = {3} (m),\ \ Area = {4} km^2 $'
    plt.text(5, -15, sim_param.format(rho, beta, '{-1}', sigma, area), size=12)
    max = 0.10

...
def main(settings, parameters, domain):
    ...
    if d:
        ...
        # subgrid size = 5(m) per point, domain size = [400, 400]
        # 1. convert to km reached
        distance_km = mean_d_metric[time_step - 1] * 5 / 1000
        # 2. workout %of one year the simulation took
        time_years = time_step / 52
        # 3. convert to km/years
        velocity_km = distance_km/time_years
        # plot end of simulation time series/
        if settings["plt_tseries"]:
            plt_tseries = Plots.plot_tseries
            saves = False
            label = {'title': "mean d", 'xlabel': 'time', 'ylabel': 'distance'}
    ...
    num_removed = len(np.where(p.removed == 1)[0]) / 4
    return num_removed, velocity_km

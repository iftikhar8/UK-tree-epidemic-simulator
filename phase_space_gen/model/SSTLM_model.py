...
class SimInit(object):
    def __init__(self, parameters, domain):
        ...
        self.sigma = parameters["sigma"]  # the 'effective' dispersal distance
        ...
        self.pre_factor = (np.sqrt(2 * np.pi) * parameters["sigma"]) ** 2  # the dispersal normalisation constant
    ...
    def get_new_infected(self, p_infected, susceptible):
        ...
        blurred_field = self.pre_factor * gaussian_filter(potential_infected, sigma=[0, self.sigma, self.sigma],
                                                          truncate=3.0)
...
def main(settings, parameters, domain):
    ...
    while in_progress:
        if verbose:
            print("Step: ", time_step, ' max: ', ts_max_d.max())
    ...
    if settings["plt_tseries"]: # GENERATE time series output plots
        ...
        print(max_distance_reached * p.alpha, '  max reached')
        print(max_mean_distance * p.alpha, ' max mean recorded')
        ts_mean_d = ts_mean_d[:time_step-1] * p.alpha
        ts_max_d = ts_max_d[:time_step-1] * p.alpha
        label = {'title': "mean d distance", 'xlabel': 'days', 'ylabel': 'distance (km)'}
        plots.plot_tseries(metric=ts_mean_d, parameters=parameters, labels=label)
        ...
        if saves:
            name = 'b_' + str(parameters["beta"]).replace('.', '-') + '_r_' + str(parameters["rho"]).replace('.', '-') + \
                   '_L_' + str(parameters["sigma"]).replace('.', '-')
